...
export function findHints() {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.match(/^\s*if\s[^:]*$/g) !== null) {
      hint += `${i + 1} 行目の「if 条件式:」の末尾のコロンを忘れていませんか？\n`;
    }
    if (line.match(/^\s*elif\s[^:]*$/g) !== null) {
      hint += `${i + 1} 行目の「elif 条件式:」の末尾のコロンを忘れていませんか？\n`;
    }
    if (line.match(/^\s*elif\s*:/g) !== null) {
      hint += `${i + 1} 行目は「elif 条件式:」か「else:」のいずれかではありませんか？\n`;
    }
    if (line.match(/^\s*else\s[^:]*$/g) !== null) {
      hint += `${i + 1} 行目の「else:」の末尾のコロンを忘れていませんか？\n`;
    }
    if (line.match(/^\s*else\s[^:]+:$/g) !== null) {
      hint += `${i + 1} 行目は「elif 条件式:」ではありませんか？\n`;
    }
    if (line.match(/^\s*for\s[^:]*$/g) !== null) {
      hint += `${i + 1} 行目の「for 変数名 in 繰り返す内容:」の末尾のコロンを忘れていませんか？\n`;
    }
    if (line.match(/^\s*while\s[^:]*$/g) !== null) {
      hint += `${i + 1} 行目の「while 条件式:」の末尾のコロンを忘れていませんか？\n`;
    }
    if (line.match(/^\s*if\s.*?:.+/g) !== null) {
      hint += `${i + 1} 行目の「if 条件式:」の末尾のコロンの後に何か書かれています．条件式をすべて書いた後にコロンを書くと直りませんか？\n`;
    }
    if (line.match(/^\s*elif\s.*?:.+/g) !== null) {
      hint += `${i + 1} 行目の「elif 条件式:」の末尾のコロンの後に何か書かれています．条件式をすべて書いた後にコロンを書くと直りませんか？\n`;
    }
    if (line.match(/^\s*while\s.*?:.+/g) !== null) {
      hint += `${i + 1} 行目の「while 条件式:」の末尾のコロンの後に何か書かれています．条件式をすべて書いた後にコロンを書くと直りませんか？\n`;
    }

    // 比較演算子
    if (line.match(/=>/g) !== null) {
      hint += `${i + 1} 行目の => は >= ではありませんか？\n`;
    }
    if (line.match(/=</g) !== null) {
      hint += `${i + 1} 行目の =< は <= ではありませんか？\n`;
    }
    ...
    if (line.match(/\w\[.{0,3}\,.{0,3}\]/g) !== null) {
      hint += `${i + 1} 行目の [ ] の間のコンマ , はスライスのコロン : ではありませんか？\n`;
    }
